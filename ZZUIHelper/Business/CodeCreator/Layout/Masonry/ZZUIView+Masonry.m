...
- (NSString *)snapkitCode {
    NSString *masonryCode = [NSString stringWithFormat:@"%@.snp.makeConstraints{ make in \n%@\n}\n", self.propertyName, self.snapkitContentCode];
    
    return masonryCode;
}

- (NSString *)snapkitContentCode {
    NSString *(^getConstantValueFromAttribute)(ZZMasonryAttribute *attribute) = ^NSString *(ZZMasonryAttribute *attribute) {
        attribute.isSnapkit = YES;
        NSString *constant;
        NSArray *temp = [attribute.constant componentsSeparatedByString:@","];
        if ([attribute.constant isPureNumber]) {
            constant = attribute.constant;
        }
        else if (attribute.attributeType == ZZLayoutAttributeTypeSize) {
            constant = [NSString stringWithFormat:@"CGSize(width: %@, height: %@)", temp[0], temp[1]];
        }
        else if (attribute.attributeType == ZZLayoutAttributeTypeCenter) {
            constant = [NSString stringWithFormat:@"CGPoint(x: %@, y: %@)", temp[0], temp[1]];
        }
        else if (attribute.attributeType == ZZLayoutAttributeTypeEdge) {
            constant = [NSString stringWithFormat:@"UIEdgeInsets(top: %@, left: %@, bottom: %@, right: %@)", temp[0], temp[1], temp[2], temp[3]];
        }
        return constant;
    };
    NSString *(^getObjectNameFromAttribute)(ZZMasonryAttribute *attribute) = ^NSString *(ZZMasonryAttribute *attribute) {
        ...
        if (attribute.object.length > 0 && ![attribute.object isEqualToString:@"superView"]) {
            NSString *object = [@"self." stringByAppendingString:attribute.object];
            if (attribute.attributeType != attribute.objectAttributeType) {
                object = [object stringByAppendingFormat:@".snp.%@", attribute.objectAttributeName];
            }
            return object;
        }
        ...
        // 约束条件不同
        if (attribute.attributeType != attribute.objectAttributeType) {
            return [self.superViewName stringByAppendingFormat:@".snp.%@", attribute.objectAttributeName];
        }
    ...
    for (ZZMasonryAttribute *attribute in self.layouts) {
        ...
        if (attribute.selected) {
            ...
            if (object) {
                ...
                if (attribute.attributeType == ZZLayoutAttributeTypeCenterX ||
                    attribute.attributeType == ZZLayoutAttributeTypeCenterY ||
                    attribute.attributeType == ZZLayoutAttributeTypeCenter) {
                    [masonryCode appendFormat:@"%@.superview!)",self.propertyName];
                } else {
                    [masonryCode appendFormat:@"0)"];
                }
            }
